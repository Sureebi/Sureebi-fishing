...
RegisterNetEvent('Sureebi_fishing:server:pickupBaitBucket', function(data)
    local src = source
...
RegisterNetEvent('Sureebi_fishing:server:grabBaitFromBucket', function(data)
    local src = source
    local ped = GetPlayerPed(src)
...
RegisterNetEvent('Sureebi_fishing:server:spawnNewBucket', function(coords, object, slot, bait)
    local src = source
    local ped = GetPlayerPed(src)
...
RegisterNetEvent('Sureebi_fishing:server:giveFish', function(fish, slot, durability)
    local src = source
    ...
    if Config.UseGroups then
        TriggerEvent('Sureebi_fishing:groups:bumpFishingTask', exports['qb-phone']:GetGroupByMembers(src))
    end
end)

RegisterNetEvent('Sureebi_fishing:server:sellFish', function() -- TODO: re-write this terrible event at some point
    local src = source
    ...
    if paymentTotal > 0 and exports.ox_inventory:AddItem(src, 'money', paymentTotal) then
        TriggerClientEvent('ox_lib:notify', src,
            { type = 'success', description = 'Спечели общо $' .. paymentTotal .. '!' })
    end
end)

RegisterNetEvent('Sureebi_fishing:server:fillBaitBucket', function(moneyToCharge)
    local src = source

    local items = exports.ox_inventory:Search(src, 'slots', 'bucket', { ['bait'] = 0 })

    if #items == 0 then
        TriggerClientEvent('ox_lib:notify', src, { type = 'error', description = 'Все още имаш примамки...' })
        return
    end
